# =============================================================
# CTV_SETUP.ps1 - CTV Trading Platform - Complete Setup  v1.9
...
# v1.5 - Added ctv_files block to manifest (hashes all .py files in C:\CTV)
# v1.6 - Fixed ConvertTo-Json -Depth 3 -> 5 (depth 3 truncated ctv_files nesting)
# v1.7 - Replaced all Write-Host with Write-Output (ps2exe -noConsole compatibility)
# v1.8 - TWS API config prompt only shown when TWS was freshly installed this run
# v1.9 - Excluded config_ctv2.py from ctv_files (tracked separately under config key)
# =============================================================
...
$configExe = "C:\CTV\config_ctv2.py"

# Build ctv_files hash table - all .py files in C:\CTV except config_ctv2.py
# (config is tracked separately under the 'config' key and updated by the Config Editor)
$ctvFiles = @{}
Get-ChildItem -Path "C:\CTV" -Filter "*.py" -File | Where-Object { $_.Name -ne "config_ctv2.py" } | ForEach-Object {
    $ctvFiles[$_.Name] = @{
        path   = $_.FullName
...
$manifest = @{
    ...
    config    = @{ path = $configExe; sha256 = (Get-SHA256 $configExe) }
    ctv_files = $ctvFiles
}
